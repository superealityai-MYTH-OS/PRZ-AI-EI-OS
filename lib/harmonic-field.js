...
exports.intentToVector = intentToVector;
/**
 ...
 */
function calculatePatternMatchConfidence(userRequest, pattern) {
 ...
 */
function calculateKeywordOverlap(tokensA, tokensB) {
 ...
 */
function calculateHarmonicAlignment(tokensA, tokensB) {
 ...
 */
function calculateIntentMagnitude(requestA, requestB) {
 ...
 */
function intentToVector(intent) {
    ...
    tokens.forEach(token => {
        if (actionVerbs.includes(token))
            direction[0] += 1;
        if (analysisVerbs.includes(token))
            direction[1] += 1;
        if (modifyVerbs.includes(token))
            direction[0] += 0.5;
    });
    // Normalize direction
